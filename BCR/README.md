...
I have used the output file of this table in conjuction with the Seurat metadata to obtain clustering information, resulting in the file `bcr_annot.csv` (code is in `clone_freq.R` in the `code` folder). Using `bcr_annot.csv`, I used the code `plot_clonal_freq.R` (which gives the same output as the function `occupiedscRepertoire()` from the `scRepertoire` package) to show clonal expansion in each cluster in each patient.
